...
class Set {
 public:
    ...
    ListedAVLTree* insert_(ListedAVLTree *t, ListedAVLTree *n, ListedAVLTree *b) {
        ...
        if (this->cmp(n->value, t->value)) {
            t->left = insert_(t->left, n, t);
        } else {
            t->right = insert_(t->right, n, b);
        }
    ...
    Set(const Set &other) : Set(other.cbegin(), other.cend()) {}
    ...
    iterator find(Key val) const {
        ...
        while (cur != nullptr) {
            if (!(this->cmp(cur->value, val) || this->cmp(val, cur->value))) {
                f.value = cur;
                break;
            } else if (this->cmp(cur->value, val)) {
                cur = cur->right;
            } else {
                cur = cur->left;
            }
